...
function PlayerPage() {
    ...
    function convertRankAndTier(tier, rank) {
        ...
        const tierAbbr = tierMap[tier.toUpperCase()] || tier; // Default to the input if no match
        const rankNum = rankMap[rank.toUpperCase()] || rank; // Default to the input if no match
    ...
    function getNextRank(tier, rank) {
        ...
        // If the rank is 1 (highest rank in tier), move to the next tier and reset to rank 4
        if (currentRankNum === 1) {
        ...
        // Otherwise, decrease the rank (e.g., G4 → G3)
        const nextRankNum = currentRankNum - 1;
    
        // Get the Roman numeral for the next rank  
        const nextRankRoman = rankRevMap[nextRankNum] || rank;
    
        return `${tierMap[tier]}${nextRankRoman}`;
    }
